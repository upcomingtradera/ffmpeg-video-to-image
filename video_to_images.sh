...
seconds_per_folder=$((frames_per_folder/(frame_rate*3)))
...
for ((start=0; start<duration; start+=seconds_per_folder)); do
    ...
    # ffmpeg -loglevel verbose -y -i "$input_video_path" -v:q 2 -vf "select=between(t\,$start\,$end),setpts=PTS-STARTPTS,fps=$frame_rate" "$folder_name/out_%09d.jpeg"
    ffmpeg -y -i "$input_video_path" -v:q 2 -vf "select=between(t\,$start\,$end),setpts=PTS-STARTPTS,fps=$frame_rate" "$folder_name/out_%09d.jpeg"
done
